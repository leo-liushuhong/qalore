...
description: >
  Token 使用统计能力。由 Claude Code Stop Hook 自动触发，不由 testcraft 主动调用。
  从 transcript JSONL 累加本轮所有 API call 的 usage 数据，以固定格式打印到对话。
  仅统计本轮；通过 last_assistant_message 信号词过滤，非 testcraft 会话静默退出；无需额外 API 调用。
---
...
本 capability 由 Claude Code **Stop Hook** 自动触发，不由 testcraft 主动调用。
testcraft SKILL.md 中注册此能力仅用于标记其存在；实际执行由 hook 完成。
...
一次 testcraft 任务包含多次 API call，各字段取值方式不同：
