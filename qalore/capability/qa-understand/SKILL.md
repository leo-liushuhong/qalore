...
渐进式阅读策略控制「同一个文件内如何读」（导航→切片精读），各适配器的「信息范围层级」章节控制「哪些文件允许读、按什么顺序读」——两者正交。示例：层级 1 允许读智能体管理 BL 文件 → 先用导航阶段扫描其 6 个功能点节头 → 后对含"创建者"字样的节做切片精读（而非全量 Read）。
...
- **多源（文本 + 代码，或多个代码仓库）** → 写入 `【qa-understand-mode: multi】` 和 `【code-repo-count: N】`（N = 本次任务的代码仓库总数）；顺序执行（不并发）：
  1. 若有文本源：Read 并执行 text.md
  2. 若有代码仓库 1..N：依次 Read 并执行 code.md（**每次执行前写入 `【code-source: {仓库名}】`**，code.md 读取 `【code-repo-count】` 和 `【code-source】` 两个标记，N=1 时使用 `[来源:code]`，N>1 时使用 `[来源:{仓库名}]`）
  3. 所有适配器完成后，Read `adapters/synthesis.md` 产出统一交接块
  - **assert_seq 续接**：每个适配器完成 ID 分配后立即更新 index.json 的 `assert_seq`，下一个适配器直接从 index.json 读取续接起点，不依赖 context 标记传递
